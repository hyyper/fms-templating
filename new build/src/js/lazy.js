...
function initSugarSrcset() {
	var attribute = 'data-srcset';

	jQuery('.img-responsive').each(function() {
		var image = jQuery(this);
		var imageUrl = image.attr('data-src');
		var suffixes = image.attr(attribute);
		var newSrcset = '';
		var separator, sufix;

		if ((imageUrl && imageUrl !== '') && (suffixes && suffixes !== '')) {
			var suffixesArr = replaceStr(suffixes).split(',');
			var imageExtension = imageUrl.slice(imageUrl.lastIndexOf('.'));
			var imageName = imageUrl.slice(0, imageUrl.lastIndexOf('.'));
			...
			for (var i = 0; i < suffixesArr.length; i++) {
				...
				if (suffixesArr[i].indexOf('x') !== -1) {
					sufix = '@';
				} else {
					sufix = '-';
				}

				newSrcset += separator + imageName + sufix + suffixesArr[i] + imageExtension + ' ' + suffixesArr[i];
			}
	
			image.attr(attribute, newSrcset);
		}
